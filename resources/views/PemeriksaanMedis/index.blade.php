<x-app-layout>
    <x-slot name="title">Pemeriksaan Medis</x-slot>
    ...
    <div x-data="pemeriksaanMedisForm()" x-on:open-add-modal.window="openAddModal()"
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            ...
            <div class="flex justify-end gap-2">
                <x-btn-success class="p-0! overflow-hidden shadow-sm">
                    <a href="{{ route('pemeriksaan_medis.excel', request()->query()) }}"
                ...
                <x-btn-danger class="p-0! overflow-hidden shadow-sm">
                    <a href="{{ route('pemeriksaan_medis.pdf', request()->query()) }}" target="_blank"
                        class="flex items-center gap-2 px-4 py-2 text-sm font-medium h-full w-full">
        ...
        <x-table>
            <thead>
                <tr>
                    ...
                    <x-th>Nama Anak</x-th>
                    <x-th>Pemberian Vitamin</x-th>
                    <x-th class="text-center">Obat Cacing</x-th>
                    <x-th class="text-center">Rujukan Medis</x-th>
                    <x-th class="text-center">Aksi</x-th>
                </tr>
            </thead>
            <tbody>
                @forelse ($pemeriksaanMedis as $item)
                <tr>
                    ...
                    <x-td>
                        <span class="inline-block px-2.5 py-0.5 rounded-full text-xs font-semibold text-white 
                                @if($item->pemberian_vitamin == 'tidak') bg-gray-600 
                                @elseif($item->pemberian_vitamin == 'vitamin_a_merah') bg-red-600 
                                @else bg-blue-600 @endif">
                            {{ ucwords(str_replace('_', ' ', $item->pemberian_vitamin)) }}
                        </span>
                    </x-td>
                    <x-td class="text-center">
                        @if ($item->pemberian_obat_cacing)
                    ...
                    <x-td class="text-center">
                        <div class="flex justify-center items-center gap-1.5">
                            ...
                            <form action="{{ route('pemeriksaan_medis.destroy', $item->pemeriksaan_id) }}"
                    ...
                    <x-td colspan="7" class="text-center p-0">
                        <div class="flex items-center justify-center py-10 w-full">
                            ...
                            <span
                                ...
                                <span>Data Pemeriksaan Medis Belum Tersedia</span>
                            </span>
                            @endif
                        </div>
                    </x-td>
        ...
        <x-paginate :paginator="$pemeriksaanMedis" />

        @include('PemeriksaanMedis.detail')
        @include('PemeriksaanMedis.modalTambah')
    </div>

    @include('PemeriksaanMedis.script')
</x-app-layout>