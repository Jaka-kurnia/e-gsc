...
<aside class="mt-4 space-y-2">
    ...
    <div class="mr-4 w-64">
        {{-- Memeriksa apakah route yang aktif adalah ibu atau anak untuk otomatis membuka dropdown --}}
        <details class="group" {{ request()->routeIs('pemeriksaan.*') || request()->routeIs('pemeriksaan_antropometri.*') ? 'open' : '' }}>
            <summary
                class="flex items-center justify-between px-4 py-3 rounded-lg ml-3 cursor-pointer list-none transition-colors
            {{ request()->routeIs('pemeriksaan.*') || request()->routeIs('pemeriksaan_antropometri.*')
                ? 'bg-indigo-600 text-white font-semibold'
                : 'text-gray-200 hover:bg-indigo-500' }}">
                <div class="flex items-center flex-1">
                    <i class="fi fi-rs-box text-lg leading-none relative top-0.5"></i>
                ...
                <span
                    class="transition transform group-open:rotate-180 {{ request()->routeIs('pemeriksaan.*') || request()->routeIs('pemeriksaan_antropometri.*') ? 'text-white' : 'text-gray-400' }}">
                    <svg class="w-4 h-4 fill-current" viewBox="0 0 20 20">
                        <path
                            d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" />
                    </svg>
